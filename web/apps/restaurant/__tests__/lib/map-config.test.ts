...
describe('restaurant public map configuration', () => {
  ...
  it('fails closed when production configuration is incomplete', () => {
    expect(() => resolvePublicMapConfig({ NODE_ENV: 'production' })).toThrow(
  ...
  it('rejects unsupported and unsafe production configuration', () => {
    expect(() => resolvePublicMapConfig({
    ...
    expect(() => resolvePublicMapConfig({
      NEXT_PUBLIC_MAP_PROVIDER: 'openfreemap',
      NEXT_PUBLIC_MAP_STYLE_URL: 'http://tiles.openfreemap.org/styles/liberty',
      VERCEL_ENV: 'production',
    })).toThrow('NEXT_PUBLIC_MAP_STYLE_URL must be a secure public URL');
    expect(() => resolvePublicMapConfig({
      NEXT_PUBLIC_MAP_PROVIDER: 'openfreemap',
      NEXT_PUBLIC_MAP_STYLE_URL: 'https://tiles.openfreemap.org/styles/liberty?token=unexpected',
      VERCEL_ENV: 'production',
