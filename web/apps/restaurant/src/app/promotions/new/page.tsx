...
type PromoType = 'discount_percent' | 'discount_amount' | 'free_delivery' | 'buy_one_get_one' | 'combo';
...
const PROMO_TYPES: { id: PromoType; label: string; desc: string }[] = [
  ...
  { id: 'buy_one_get_one', label: 'Mua 1 tặng 1', desc: 'Tặng món miễn phí khi mua một món khác' },
  { id: 'combo', label: 'Combo ưu đãi', desc: 'Gói combo với giá đặc biệt' },
];
...
export default function PromotionNewPage() {
  ...
  const [form, setForm] = useState<PromoFormData>({
    name: '', description: '', type: 'discount_percent', discountValue: 10,
    startDate: '', endDate: '', minOrderValue: 0, maxUses: 100, code: '',
  });
  ...
  const validateStep = (): string => {
    if (step === 0) {
      ...
      if (form.type !== 'free_delivery' && form.discountValue <= 0) return 'Vui lòng nhập giá trị giảm giá hợp lệ';
      if (form.type === 'discount_percent' && form.discountValue > 100) return 'Phần trăm giảm không được vượt quá 100%';
    }
    if (step === 1) {
  ...
  const handleNext = () => {
    ...
    setStepError('');
    if (step === 2 && !form.code) update('code', generatePromoCode(form.name));
    setStep((s) => s + 1);
  };
  ...
  const handleSubmit = async () => {
    ...
    setStepError('');
    try {
      await api.post('/restaurant/promotions', { ...form, code: finalCode });
      router.push('/promotions');
    } catch (err: unknown) {
      setStepError((err as { message?: string }).message || 'Không thể tạo khuyến mãi');
    } finally {
      setIsSubmitting(false);
    }
  };

  const typeLabel = PROMO_TYPES.find((t) => t.id === form.type)?.label ?? '';
  const reviewRows: [string, string][] = [
    ['Tên', form.name],
    ['Loại', typeLabel],
    ...(form.type !== 'free_delivery' ? [[form.type === 'discount_percent' ? 'Giảm %' : 'Giảm tiền',
      `${form.discountValue}${form.type === 'discount_percent' ? '%' : ' VNĐ'}`] as [string, string]] : []),
    ['Bắt đầu', form.startDate],
    ['Kết thúc', form.endDate],
    ...(form.minOrderValue > 0 ? [['Đơn tối thiểu', `${form.minOrderValue.toLocaleString('vi-VN')} VNĐ`] as [string, string]] : []),
  ...
  return (
    <div className="animate-fade-in-up">
      <button onClick={() => router.push('/promotions')} className="btn-ghost mb-4 -ml-2">
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            ...
            <span className={cn('text-sm hidden sm:inline', i === step ? 'text-gray-900 font-medium' : 'text-gray-400')}>
              {s}
            </span>
            {i < STEPS.length - 1 && (
