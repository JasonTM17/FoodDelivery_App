...
export function resolvePublicMapConfig(
  ...
  if (!provider) {
    throw new Error('NEXT_PUBLIC_MAP_PROVIDER is required for the admin driver map');
  }
  ...
  if (!configuredStyleUrl && production) {
    throw new Error('NEXT_PUBLIC_MAP_STYLE_URL is required for the admin driver map');
  }
