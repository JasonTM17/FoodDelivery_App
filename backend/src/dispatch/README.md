...
- WebSocket `/dispatch`: server emits driver-only offers `driver:new_order` with `{ orderId, offerToken, restaurantName, restaurantAddress, deliveryAddress, orderTotal, deliveryFee, distanceKm, timeoutSeconds, surgeMultiplier }`; drivers reply with `dispatch:accept` / `dispatch:reject` using `{ orderId, offerToken }`; server emits `driver:order_assigned` to the assigned driver after persistence.
- WebSocket `/events`: order-facing dispatch outcomes are emitted through `OrdersGateway` rooms that customers/restaurants can subscribe to. `driver:assigned` carries `{ driverId, etaMinutes: null }`; routed ETA must come from tracking `delivery:eta_updated` after Google/OSRM route data is available, not from speed-based distance heuristics. `order:auto_cancelled` carries `{ orderId, reason: "no_driver_available" }`.
