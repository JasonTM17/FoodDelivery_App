...
これらは device/emulator 用の native command であり、local web URL ではありません。Android flavor は `mobile/android/app/build.gradle.kts` に定義されています。iOS development では対応する Dart entrypoint を使います。
