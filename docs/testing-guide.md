...
- Backend passed `pnpm typecheck`, `pnpm lint`, full `pnpm test` (107 suites / 758 tests), and `pnpm build`; focused dispatch/order-code regressions passed 3 suites / 46 tests.
...
- Dispatch/map evidence: restaurant acceptance now enqueues route-aware dispatch jobs with restaurant latitude/longitude and attempt metadata; the worker skips legacy malformed jobs, parses ioredis `GEOSEARCH WITHDIST` tuple rows, and no longer fails with `raw[i].replace is not a function`.
