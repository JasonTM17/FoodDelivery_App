...
Current runtime health evidence is tied to SHA `977d55f19ddc4fecafb8a758d2df034f4b6ff21d`. Railway API/worker/migrator and both Vercel apps report that revision, while database, Redis, and Supabase Storage are up. The full release, registry, SBOM, multi-architecture, and provider-smoke evidence remains bound to tagged release SHA `a703ece61e66dcfe7f308cbf46a98098983233e7`; it must not be presented as certification of `977d55f`. The current branch counts and historical role/browser results below remain bounded evidence, not full end-to-end production approval.
