...
| Database       | The historical disposable baseline applied its then-current 38 migrations and rejected a second default address for one user. The current read-only production audit found 45 migration records: 41 effective finished and 4 rolled back, zero business/retrieval rows, and only the intended public/private Storage buckets. Three applied checksum provenance mismatches remain unresolved.                                                                                               |
