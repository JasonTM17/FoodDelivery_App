...
Latest current-head map/tracking evidence: 2026-07-04 at `3252c6a`, backend `pnpm install --frozen-lockfile`, `pnpm typecheck`, `pnpm lint`, full `pnpm test` (106 suites / 752 tests), focused `route-utils.spec.ts` (11 tests), and `pnpm build` all passed. Admin driver map realtime validation passed `pnpm --filter foodflow-admin test` (34 files / 139 tests), `pnpm --filter foodflow-admin typecheck`, `pnpm --filter foodflow-admin lint`, web `pnpm install --frozen-lockfile`, and `pnpm --filter foodflow-admin build`. Mobile customer/driver maps now share a Vietnam delivery coordinate guard before drawing markers, telemetry trails, route polylines, or opening Google Maps directions; focused map/route Flutter tests passed 14/14, `flutter analyze` found no issues, and full `flutter test` passed 143 tests.
...
Latest current-head backend/web gate evidence: 2026-07-04 at `03ffed4`, backend `pnpm audit`, `pnpm install --frozen-lockfile`, `pnpm prisma generate`, `pnpm typecheck`, `pnpm lint`, full `pnpm test` (106 suites / 752 tests), and `pnpm build` all passed after dependency advisory patches. Web workspace `pnpm audit --audit-level moderate`, `pnpm install --frozen-lockfile`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (Admin 34 files / 139 tests; Restaurant 27 files / 79 tests), and `pnpm build` all passed after the Next 15 / next-intl 4 security migration. The Admin build generated 70 localized pages and the Restaurant build generated 55 localized pages across `vi`, `en`, and `ja`.
...
Latest local mobile evidence: 2026-07-04 at `3252c6a` on `codex/batch4-integration`, `flutter pub get --enforce-lockfile` passed, focused map/route tests passed 14/14 (`directions_uri_test.dart`, `trip_route_provider_test.dart`, `encoded_polyline_test.dart`, `tracking_provider_test.dart`, `lat_lng_validation_test.dart`), `flutter analyze` found no issues, and full `flutter test` passed 143 tests. Backend route integrity gates also passed locally for the same map/tracking cluster: `pnpm typecheck`, `pnpm lint`, full `pnpm test` (106 suites, 752 tests), focused `route-utils.spec.ts`, and `pnpm build`. `flutter build apk --debug` is not yet a valid gate for this repo because `mobile/` currently has no `android/` platform project or `.metadata`; Flutter reports an unsupported Gradle project before compilation. Generate/reconcile the Android/iOS platform projects in the dedicated mobile phase before requiring APK/IPA builds.
