...
| Area           | Result                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Backend        | Historical runtime candidate `f2c02ed76fb6a79671c1c51d10d8b6aef0f55b8b` passed 145 suites / 1071 tests, `tsc --noEmit`, ESLint, and the Nest build. Current source branch `ea89102d6059d2d9102b73acae892629018b791f` passes the focused migration suite (5/5), lint, typecheck, and build; Railway currently runs `7a015056` without the new guard. |
| Database       | The historical disposable baseline applied its then-current 38 migrations and rejected a second default address for one user. The current read-only production audit found 45 migration records: 41 effective finished and 4 rolled back, zero business/retrieval rows, and only the intended public/private Storage buckets. The three historical checksum records are reconciled from immutable artifact/Git evidence; the source branch adds a fail-closed guard, while the currently running image still predates it. See [reconciliation](migration-checksum-reconciliation.md). |
| Mobile Flutter | The current local run passed 369 tests and `flutter analyze` with no issues. Device-specific Android/iOS production evidence is still required.                                                                                                                                                                                                                                                                       |
