...
Runtime SHA `977d55f19ddc4fecafb8a758d2df034f4b6ff21d` is the current deployed application revision. Railway migrate/API/worker deployments `e100789f-03c1-445d-9e69-b8a243973a95`, `a84c63d1-c95e-4a69-a7eb-408e1a7dc9f4`, and `2e4a41ea-6874-4b01-b549-d457c0a20997` report success. API health/readiness and public web health for Vercel Admin `dpl_bE5TgrKS9GqKGHSShGHk1pX41Xqs` and Restaurant `dpl_J6sXb2UHV68XKAYBF4KLvqoXAjwz` return this exact revision. Database, Redis, and Supabase Storage are ready. Production has all 42 repository migrations active; the 46-row Prisma history retains four rolled-back audit rows.

Runtime SHA `977d55f19ddc4fecafb8a758d2df034f4b6ff21d` is also the tagged [`v0.1.3`](https://github.com/JasonTM17/FoodDelivery_App/releases/tag/v0.1.3) release. Docker Publish run `29490699451` passed its release gates and promoted immutable SHA, `v0.1.3`, and `latest` aliases on Docker Hub and public GHCR. Release run `29490929946` revalidated the tag/registry evidence and published exactly `CHANGELOG.md`, `sbom-source.spdx.json`, and `sbom-image.spdx.json`.

A controlled synthetic HCMC production run against current SHA `977d55f` authenticated temporary Admin/Driver identities, verified a five-minute ES256 realtime token, allowed the Admin channel and denied the Driver cross-scope channel through private Broadcast RLS, accepted a GPS sample with no fabricated order, received Broadcast in 1,271 ms, and persisted the PostGIS row. It also proved `poor_accuracy` and `driver_offline` rejection, exact fixture cleanup, zero users/restaurants/GPS rows, and completed lifecycle state. No real coordinates or personal data were used. Physical Android/iOS background location, controlled FCM delivery, full current-revision role journeys, active-order routing, and optional providers remain open.
...
The controlled four-role smoke and its Admin/Restaurant Chrome screenshots were captured earlier at SHA `17584153ff256b74a3413ae9844f4f27bff038cc`. It remains historical evidence for that exact revision, not authentication certification for current SHA `977d55f`. The current-revision GPS/private Broadcast/PostGIS evidence is API/realtime production evidence, not a physical-device certification or full four-role UI journey. Migration 42 and its hardened recovery controller are deployed. Full certification remains open for current-revision four-role journeys, physical Android/iOS background location, controlled FCM delivery, active-order routing, and optional providers.

### Post-merge candidate gates — 2026-07-16

The reconciled branch contains current production history through `977d55f` plus migration provenance, fixture-lease, dependency, workflow, disk-guard, and media-integrity hardening. Backend Prisma generate/validate, typecheck, ESLint, and Nest build pass. Full Jest reports 156 passing suites, one gated integration suite skipped, 1,178 passing tests, and one skipped test. Focused migration/fixture suites pass 62 tests with one gated integration suite skipped; the production role suite passes 46 tests with one destructive integration test skipped. A fresh marked disposable PostGIS+pgvector database previously applied all 42 migrations, and the remote read-only audit now reports 42 active migrations with no unresolved failure.
...
Three already-applied migration files have checksums that do not match retained source history: `20260709143000_add_realtime_outbox`, `20260709150000_add_job_outbox`, and `20260712143000_add_production_storage_bucket`. No matching source was found in retained Git history or the eight local stashes. Their schema/index/RLS end-state matches the repository, and neither applied files nor remote records were rewritten. The production checksum guard now pins both the exact recorded production digest and the reviewed canonical local digest for each of these three rows; only that pair and LF/CRLF variants pass, while changing either side fails closed before provider/schema mutation. The read-only production audit reports checksum status `ok`, 42 active applied migrations, and no unresolved migration. Do not use `prisma migrate resolve` to conceal future drift. Supabase advisors report only the known `postgis` and `vector` extension-in-public warnings. These non-relocatable/provider-managed extension warnings remain documented rather than being changed destructively.
...
The 2026-07-16 decision remains **NO-GO for full certification**. Current SHA `977d55f` has exact API/readiness and public Admin/Restaurant web-health evidence, all 42 source migrations active, ready Database/Redis/Supabase Storage, immutable `v0.1.3` release images, and a current-revision API-level GPS/private Broadcast/PostGIS smoke. Full approval still requires current-`977d55f` authenticated Customer/Driver/Restaurant/Admin UI journeys, active-order tracking, controlled FCM delivery, Android/iOS background-location physical-device evidence, KYC private-object smoke, and any optional integration included in the certified feature set. No secret, seed, ETA, provider answer, or embedding may be invented or bypassed to change that decision.
