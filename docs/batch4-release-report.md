...
Production Storage checksum `4664ac4299eea854a16316be6a9ed689a3320c1fca2557a4fd00f011368fd8e6`, applied at `2026-07-12T01:08Z` for `20260712143000_add_production_storage_bucket`, was not found in retained Git/agent history or any inspected registry image. It is the sole remaining checksum-provenance blocker. The current read-only production audit exits `1` naming only that migration, before Storage or schema mutation. A matching schema end-state cannot prove the original SQL bytes; do not use `prisma migrate resolve` to conceal it. All 42 source migrations are already active in production; this guard blocks the next migrator rollout, not the health of the current schema. Supabase advisors report only the known `postgis` and `vector` extension-in-public warnings; these non-relocatable/provider-managed constraints remain documented instead of being changed destructively.
