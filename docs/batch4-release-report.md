...
Runtime SHA `977d55f19ddc4fecafb8a758d2df034f4b6ff21d` is the current deployed application revision. Railway migrate/API/worker deployments `e100789f-03c1-445d-9e69-b8a243973a95`, `a84c63d1-c95e-4a69-a7eb-408e1a7dc9f4`, and `2e4a41ea-6874-4b01-b549-d457c0a20997` report success. API health/readiness and authenticated web health for Vercel Admin `dpl_bE5TgrKS9GqKGHSShGHk1pX41Xqs` and Restaurant `dpl_J6sXb2UHV68XKAYBF4KLvqoXAjwz` return this exact revision. Database, Redis, and Supabase Storage are ready; production has 41 applied database migrations, while candidate migration 42 remains undeployed.

Historical release baseline `a703ece61e66dcfe7f308cbf46a98098983233e7` remains the tagged `v0.1.2` release. Docker Publish run `29474270122` reused its verified immutable manifests, passed four multi-architecture runtime smokes, eight architecture-specific Trivy scans, and its production smoke, then promoted `v0.1.2` and `latest`. Release run `29478484699` revalidated the Git tag and registry digests after the duplicate-SBOM fix, generated the changelog and both SBOMs, and updated [GitHub Release v0.1.2](https://github.com/JasonTM17/FoodDelivery_App/releases/tag/v0.1.2) with exactly three assets. This Docker/release evidence belongs to `a703ece`; it is not a build or smoke claim for current SHA `977d55f`.

The historical `a703ece` production smoke authenticated temporary Admin/Driver accounts, verified five-minute ES256 realtime tokens, private Broadcast RLS allow/deny, accepted GPS fanout, one persisted PostGIS row, `poor_accuracy` and `driver_offline` rejection reasons, and complete DB/Redis cleanup. A read-only follow-up inventory found zero users, Customer/Driver/Restaurant profiles, restaurants, orders, and GPS rows. Admin health was public. Restaurant health was verified through authenticated Vercel access because its project uses `all_except_custom_domains` SSO protection and has no custom domain; unauthenticated requests redirected to Vercel SSO, so public Restaurant availability was not certified. Docker Hub and public GHCR `sha-a703ece...`, `v0.1.2`, and `latest` tags are digest-equal for backend, migrator, Admin, and Restaurant. The full four-role, GPS, and device certification has not been rerun against current SHA `977d55f`; physical Android/iOS background location, controlled FCM delivery, full authenticated role journeys, and optional providers included in certification scope remain open.
...
The controlled four-role smoke and its Admin/Restaurant Chrome screenshots were captured earlier at SHA `17584153ff256b74a3413ae9844f4f27bff038cc`. It remains historical evidence for that exact revision, not authentication certification for current SHA `977d55f`. The GPS/private Broadcast/PostGIS smoke described above likewise remains bounded `a703ece` release evidence, not a current-revision rerun. Candidate migration 42 and its hardened recovery controller remain undeployed pending PR review, backup, synchronized rollout, and post-deploy verification. Full certification remains open for current-revision four-role journeys, public Restaurant access, physical Android/iOS background location, controlled FCM delivery, active-order routing, and optional providers.

### Post-merge candidate gates — 2026-07-16

The reconciled branch contains current production history through `977d55f` plus the undeployed migration 42/controller hardening. Backend Prisma generate/validate, typecheck, ESLint, and Nest build pass. Full Jest reports 156 passing suites, one gated integration suite skipped, 1,179 passing tests, and one skipped test. The newly merged GPS/realtime/Redis tests pass 101/101; role-smoke unit tests pass 44/44. A fresh marked disposable PostGIS+pgvector database applied all 42 migrations, then the destructive role-smoke integration passed 1/1 and returned `users=0`, `restaurants=0`, and `smoke_runs=0`; the container was removed.
...
Production Storage checksum `4664ac4299eea854a16316be6a9ed689a3320c1fca2557a4fd00f011368fd8e6`, applied at `2026-07-12T01:08Z` for `20260712143000_add_production_storage_bucket`, was not found in any Git object or inspected registry image. It is the sole remaining checksum-provenance blocker. The current read-only production audit exits `1` naming only that migration, before Storage or schema mutation. A matching schema end-state cannot prove the original SQL bytes; do not use `prisma migrate resolve` to conceal it. Candidate migration 42 remains undeployed. Supabase advisors report only the known `postgis` and `vector` extension-in-public warnings; these non-relocatable/provider-managed constraints remain documented instead of being changed destructively.
...
The 2026-07-16 decision remains **NO-GO for full certification**. Current SHA `977d55f` has exact API/readiness and authenticated Admin/Restaurant web-health evidence, 41 applied database migrations, and ready Database/Redis/Supabase Storage. Immutable release images and the API-level GPS/private Broadcast/PostGIS path remain verified historical `a703ece` evidence, not a current-revision rerun. Full approval still requires current-`977d55f` authenticated Customer/Driver/Restaurant/Admin journeys, GPS and active-order tracking smoke, controlled FCM delivery, Android/iOS background-location device evidence, KYC private-object smoke, and any optional integration included in the certified feature set. No secret, seed, ETA, provider answer, or embedding may be invented or bypassed to change that decision.
