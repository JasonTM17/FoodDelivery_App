...
**Remote status supersession:** Railway currently runs revision `7a0150567c3e408b8ba045faaf781044257ba05c` (`foodflow-api` deployment `30a3e03d-1f95-4456-93b9-bbaa8d8c4c17`, worker `6be4ccdb-5fa7-4bc3-8e8f-b1282bda74f9`, one-off migrator `b8c31114-1140-427d-85cf-bb4176e415c9`). Health and readiness return 200 with DB, Redis, and Supabase Storage up; the read-only migration query still returns the three documented production checksums. This current image predates source-branch guard commit `b5773c4`, so the `f2c02ed` runtime/image evidence below is historical and cannot be used to claim the guard is live. Deploy the guard only from a new immutable SHA after CI, image scan, backup, and migrator preflight.
...
The three historical checksum records are now reconciled without mutating Supabase: realtime and job outbox SQL were recovered from immutable `foodflow-migrate:sha-1f761a65b4a7053858a512bf6eb09a3fd2adbef0`, while the Storage SQL was recovered from Git object `c29c069ea180ed6c3107411759b8ceb2150dc8e7`. The exact production/source pairs and the byte-identical Storage source are recorded in [migration checksum reconciliation](migration-checksum-reconciliation.md). The source branch adds a fail-closed checksum guard before every future provider/schema mutation and focused coverage for the approved pair and rejection path. The currently running Railway image predates this guard, so production protection is not claimed until a new immutable image is built, scanned, deployed, and preflighted. Do not use `prisma migrate resolve` to conceal an unknown checksum drift. Supabase advisors report only the known `postgis` and `vector` extension-in-public warnings. These non-relocatable/provider-managed extension warnings remain documented rather than being changed destructively.
