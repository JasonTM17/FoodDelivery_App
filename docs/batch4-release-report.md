...
## Current production runtime evidence — 2026-07-17

Runtime SHA `84eeac3a2845868fc3a7fd45f8a73775e834a09d` remains the current Railway application revision. After the Supabase database password rotation, Railway migrate/API/worker deployments `e61a23bc-ce7e-4ef7-9daa-12160e20f105`, `f4292a62-4497-4f7d-9f8d-1c53bb2ca938`, and `e654a826-6255-4402-aed8-af57cd4fcd67` report success. The migrator found 42 migrations with none pending; the checksum audit reports `42/42`; API health/readiness return HTTP 200 with Database, Redis, and Supabase Storage up; API and worker startup logs contain no error patterns. Admin and Restaurant Vercel canonical health now both report exact source `e6def517334681f3e003685489bd190e72408344`, and their public login surfaces pass. The quota boundary is resolved. The merged release helper now also refuses a web rollout unless Railway API health reports the same full SHA first.

Docker Publish run `29515529360` built, runtime-smoked, scanned, and published the four immutable `sha-84eeac3a2845868fc3a7fd45f8a73775e834a09d` manifests to Docker Hub and public GHCR. All four packages are public. API and worker share `foodflow-backend`; there is deliberately no duplicate GHCR worker package. Semver and `latest` aliases remain on the prior certified release until a later immutable candidate is deployed and passes the same provider gates.
