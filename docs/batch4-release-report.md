...
Runtime SHA `a703ece61e66dcfe7f308cbf46a98098983233e7` is the current deployed application revision. All push workflows are green, and Docker Publish SHA run `29411588074` passed four multi-architecture runtime smokes plus eight architecture-specific Trivy scans. Railway migrate/API/worker deployments `49579ce7-9808-4a35-afcc-82432943bc70`, `9c823cd9-290a-4eb0-94a2-fdf01c3f0b06`, and `413dedcc-6ba7-46be-8c99-901f592c558f` report success; API health/readiness and Vercel deployments `dpl_7CFZKPxtNsYeF1Y6BZmnoJEoXyiF` and `dpl_6jqguNYtbVCMVaQ6GvikiceYVsGN` return this exact revision. Supabase has 41 effective migrations with none pending and healthy PostgreSQL/Redis/Storage.

The final production smoke authenticated temporary Admin/Driver accounts, verified five-minute ES256 realtime tokens, private Broadcast RLS allow/deny, accepted GPS fanout, one persisted PostGIS row, `poor_accuracy` and `driver_offline` rejection reasons, and complete DB/Redis cleanup. Docker Hub SHA, `v0.1.2`, and `latest` tags are digest-equal for backend, migrator, Admin, and Restaurant. GHCR exposes all four public, digest-equal SHA packages; GHCR semver/latest is not claimed because both repository and local package-write tokens returned `401 Unauthorized` for manifest writes. Full device certification remains open for physical Android/iOS background-location, controlled FCM delivery, full authenticated browser role journeys, and optional providers included in certification scope.

Manual release run `29413125555` passed preflight, all four runtime smokes, all eight image scans, and production smoke, then its promotion step exited before tagging. Idempotent recovery promoted and re-read all four Docker Hub `v0.1.2` and `latest` manifests from the already verified SHA images; no image was rebuilt. The workflow now keeps Docker Hub promotion strict and authoritative while reporting only GHCR authorization refusal as a non-fatal provider limitation. Digest conflicts and Docker Hub failures still fail the release.

### Vercel revision drift and recovery — 2026-07-15

The final backend-only fix was intentionally skipped by each app's Vercel `ignoreCommand`. A 2.3 MB tracked-source staging archive removed only that command outside the repository, injected `BUILD_SHA=a703ece...`, and created non-aliased production deployments. Admin and Restaurant were promoted only after both were `Ready`; canonical health returns the same full SHA as Railway and public login smoke passes for `vi`, `en`, and `ja`. Chrome DOM smoke confirmed the Vietnamese Admin and Restaurant forms. The staging directory and temporary GPS script were deleted; the worktree returned clean before this report update.
...
The API environment explicitly selects `REALTIME_PROVIDER=supabase` and has the publishable, secret, service-role JWT, ES256 private key, and key ID configured. Supabase has RLS enabled on `realtime.messages`, `realtime_outbox`, and `driver_location_history`; the private Broadcast SELECT policy requires the topic to appear in the token's `realtime_channels` claim, and `realtime_outbox` is absent from the legacy `supabase_realtime` publication. The previous controlled authenticated production GPS smoke remains valid because tracking/realtime code is unchanged: it obtained a five-minute scoped token, submitted simulated HCMC GPS through Railway, received private `admin:drivers` Broadcast, persisted PostGIS history in 1437 ms, and cleaned all temporary rows.
