...
Runtime SHA `a703ece61e66dcfe7f308cbf46a98098983233e7` is the current deployed application revision. Railway migrate/API/worker deployments `49579ce7-9808-4a35-afcc-82432943bc70`, `9c823cd9-290a-4eb0-94a2-fdf01c3f0b06`, and `413dedcc-6ba7-46be-8c99-901f592c558f` report success; the API/ready health and both Vercel health endpoints return this exact revision. Admin health is public. Restaurant health was verified through the authenticated Vercel CLI because its project uses `all_except_custom_domains` SSO protection and currently has no custom domain; unauthenticated requests redirect to Vercel SSO, so public Restaurant availability is not certified. A read-only production inventory found 41 applied migrations and zero users, Customer/Driver/Restaurant profiles, restaurants, orders, and GPS rows; PostgreSQL, Redis, and Supabase Storage are ready. Railway pins the API/worker to immutable backend tag `sha-a703ece61e66dcfe7f308cbf46a98098983233e7`; image digests were verified through provider state without copying credentials into this report.

The controlled role smoke and its Admin/Restaurant Chrome screenshots were captured earlier at SHA `17584153ff256b74a3413ae9844f4f27bff038cc`. That run exercised Customer/Driver read-only API contracts, proved cross-role denial and protected-route session rejection, and returned production to zero business rows. It remains valid historical evidence for that exact revision, not authentication certification for current SHA `a703ece`. Full certification remains open for current-revision authenticated roles, real Android/iOS background-location, controlled FCM delivery, active-order routing, and optional provider credentials. Candidate migration 42 and its hardened recovery controller remain undeployed pending PR review, backup, synchronized rollout, and post-deploy verification.
