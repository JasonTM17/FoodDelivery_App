...
Current runtime health evidence は SHA `977d55f19ddc4fecafb8a758d2df034f4b6ff21d` に紐づきます。Railway API/worker/migrator と両 Vercel apps はこの revision を返し、database、Redis、Supabase Storage は up です。Full release、registry、SBOM、multi-architecture、provider smoke の evidence は tagged release SHA `a703ece61e66dcfe7f308cbf46a98098983233e7` に紐づくままで、`977d55f` の certification ではありません。以下の current branch count と historical role/browser result は範囲を限定した evidence であり、完全な end-to-end production approval ではありません。
...
| Production     | Railway migrator `e100789f-03c1-445d-9e69-b8a243973a95`、API `a84c63d1-c95e-4a69-a7eb-408e1a7dc9f4`、worker `2e4a41ea-6874-4b01-b549-d457c0a20997` は SHA `977d55f` で成功。Vercel Admin `dpl_bE5TgrKS9GqKGHSShGHk1pX41Xqs` と Restaurant `dpl_J6sXb2UHV68XKAYBF4KLvqoXAjwz` も同じ revision です。API health/readiness、public Admin health、authenticated Restaurant health は exact SHA を返します。Controlled Admin/Driver GPS/Supabase smoke は以前の SHA `a703ece` の evidence であり、current runtime の certification ではありません。Public Restaurant request は Vercel SSO に redirect され、public availability と current four-role certification は未完了です。 |
