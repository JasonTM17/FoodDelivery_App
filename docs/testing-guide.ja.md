...
以前の map/tracking evidence: 2026-07-04 `3252c6a` で、backend `pnpm install --frozen-lockfile`、`pnpm typecheck`、`pnpm lint`、full `pnpm test` (106 suites / 752 tests)、focused `route-utils.spec.ts` (11 tests)、`pnpm build` はすべて pass。Admin driver map realtime は `pnpm --filter foodflow-admin test` (34 files / 139 tests)、`pnpm --filter foodflow-admin typecheck`、`pnpm --filter foodflow-admin lint`、web `pnpm install --frozen-lockfile`、`pnpm --filter foodflow-admin build` が pass。2026-07-05 の追加 evidence では、customer mobile が realtime event 前に REST snapshot `/orders/:id/tracking` を hydrate し、`routePhase` を扱い、snapshot に route が無い場合は stale route geometry を clear し、invalid driver GPS を拒否し、real route cache が無い時に persisted estimated minutes を表示しないことを検証しました。`flutter analyze`、full `flutter test` (166 tests)、backend tracking suite (5 suites / 41 tests)、focused dispatch/tracking regressions (2 suites / 16 tests)、web typecheck、OpenAPI/Spectral checks が pass。
...
最新の local mobile evidence: 2026-07-05 / `codex/batch4-integration` で、verified runtime code includes `d201ce1`。`flutter pub get --enforce-lockfile` は pass、`flutter analyze` は issue 0、full `flutter test` は 166 tests pass、`dart analyze mobile/packages/api_client` も issue 0。Android flavor debug APK build は `flutter build apk --debug --flavor customer -t lib/main_customer.dart` と `flutter build apk --debug --flavor driver -t lib/main_driver.dart` の両方が passし、customer/driver は package ID と launcher label を分離しています。`d201ce1` driver map patch は trip route replay と demand heatmap を backend route/demand coordinates から native Google Maps overlays で render します。Targeted `dart analyze lib\driver\widgets\route_replay_map.dart lib\driver\widgets\heatmap_canvas.dart`、full `flutter analyze`、full `flutter test`、driver debug APK build は patch 後に pass。Native Google Maps key は env/local xcconfig のみから読み、Android release signing は `FOODFLOW_UPLOAD_*` secrets が揃うまで fail-closed です。同じ map/tracking cluster で backend route integrity gates も local pass 済みです: `pnpm typecheck`、`pnpm lint`、`pnpm build`、backend tracking suite (5 suites / 41 tests)、focused dispatch/tracking regressions (2 suites / 16 tests)。
...
Verified runtime code includes `d201ce1`; Docker/E2E was rerun after docs head `e24631c`, and the mobile driver Google Maps overlay patch was verified at `d201ce1`。Remote `codex/batch4-integration` は `3857433` で patch-equivalence 確認後に削除されました。Clean local worktree は local `codex/batch4-integration` を使う場合がありますが、`origin/master` を tracking しています。GitHub token/auth/billing が未解決のため、remote CI/Actions は pending です。docs-only evidence commits 後の正確な current `master` SHA は `git ls-remote --heads origin` で確認します。
