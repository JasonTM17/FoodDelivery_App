...
以前の map/tracking evidence: 2026-07-04 `3252c6a` で、backend `pnpm install --frozen-lockfile`、`pnpm typecheck`、`pnpm lint`、full `pnpm test` (106 suites / 752 tests)、focused `route-utils.spec.ts` (11 tests)、`pnpm build` はすべて pass。Admin driver map realtime は `pnpm --filter foodflow-admin test` (34 files / 139 tests)、`pnpm --filter foodflow-admin typecheck`、`pnpm --filter foodflow-admin lint`、web `pnpm install --frozen-lockfile`、`pnpm --filter foodflow-admin build` が pass。2026-07-05 `d5ecfcb` の追加 evidence では、customer mobile が realtime event 前に REST snapshot `/orders/:id/tracking` を hydrate し、`routePhase` を扱い、snapshot に route が無い場合は stale route geometry を clear し、invalid driver GPS を拒否することを検証しました。`flutter analyze`、full `flutter test` (149 tests)、focused tracking provider tests (6/6)、backend tracking controller tests (3/3)、web typecheck、OpenAPI/Spectral checks が pass。
