...
最新の local mobile evidence: 2026-07-05 / `codex/batch4-integration` で、`flutter pub get --enforce-lockfile` は pass、`flutter analyze` は issue 0、full `flutter test` は 166 tests pass、`dart analyze mobile/packages/api_client` も issue 0。Android flavor debug APK build は `flutter build apk --debug --flavor customer -t lib/main_customer.dart` と `flutter build apk --debug --flavor driver -t lib/main_driver.dart` の両方が passし、customer/driver は package ID と launcher label を分離しています。Native Google Maps key は env/local xcconfig のみから読み、Android release signing は `FOODFLOW_UPLOAD_*` secrets が揃うまで fail-closed です。同じ map/tracking cluster で backend route integrity gates も local pass 済みです: `pnpm typecheck`、`pnpm lint`、`pnpm build`、backend tracking suite (5 suites / 41 tests)、focused dispatch/tracking regressions (2 suites / 16 tests)。
