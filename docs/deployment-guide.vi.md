...
Tạo Railway services `foodflow-api` (root `backend`, `backend/railway.toml`), `foodflow-worker` (backend image cùng SHA, command `dist/workers/main.js`), `foodflow-migrate` (migrate image cùng SHA) và managed Redis. Chạy migrator một lần sau backup Supabase và trước API; chỉ deploy Admin/Restaurant trên Vercel. Image migrator chạy `dist/migrations/production-migrate.js`: trước hết kiểm tra checksum mọi migration đã áp dụng, chấp nhận biến thể LF/CRLF và đúng ba cặp checksum remote/local lịch sử được ghim; sau đó dùng `SUPABASE_SERVICE_ROLE_KEY` dạng JWT để xoá qua Storage API đúng hai bucket legacy (Supabase sẽ từ chối nếu bucket có object), chỉ resolve bản ghi migration bucket rỗng đã fail sau khi cleanup thành công, rồi chạy `prisma migrate deploy`. Checksum chưa được duyệt, thay đổi bất kỳ phía nào của cặp đã ghim hoặc lỗi inventory/delete bucket sẽ fail-closed trước rollout schema. Không dùng `prisma migrate resolve` để che provenance drift.
...
### Smoke role có xác thực khi production chưa có dữ liệu

User của FoodFlow dùng JWT/Prisma riêng, không phải Supabase Auth. Dùng `backend/scripts/production-role-smoke-fixture.ts` để tạo đúng bốn identity tạm `@example.invalid` và một restaurant có ownership marker, inactive/closed. Phải truyền confirmation exact, run ID duy nhất, mật khẩu ASCII in được, signal path tuyệt đối và cleanup-trigger deadline 60–900 giây bắt đầu trước provision; `READY` in số giây còn lại, thời gian cleanup và capability drain tính thêm. Chạy qua service `foodflow-migrate`. Script chỉ chạy khi Railway environment là `production`, `DATABASE_URL` khớp project ref của `SUPABASE_URL`, dùng database `postgres`, schema `public`, direct/session port `5432` (không dùng transaction-pool `6543`), và serializable preflight thấy `0` user/restaurant/order/GPS. Script không in URL/row/credential/token, giới hạn Prisma `connection_limit=1`, giữ advisory lease và heartbeat đúng PostgreSQL backend PID.

Provision ghi một lifecycle row không chứa secret gồm run ID và các UUID bất biến trong cùng transaction với fixture. Sau dòng `READY`, dùng Google Chrome đăng nhập Admin/Restaurant và chỉ chạy API read-only cho Customer/Driver: profile, orders/earnings, private Realtime và cross-role denial. Không tạo order, payment, export, AI session, FCM token, upload hay GPS. Tạo file signal ngay khi test xong. Realtime signer giữ shared lock trên user đang active; cleanup khóa lifecycle + fixture, revalidate ownership/topology trong cùng serializable transaction, kiểm tra lại lease PID, dùng FK để chặn semantic orphan, chỉ xóa bằng UUID và atomically ghi state `deletion_committed`. Run bình thường phải thấy `CLEANUP_OK ... outcome=deleted`, sau đó kiểm tra protected route trả về login, đóng Chrome và mọi Realtime client. Controller giữ nguyên lease/PID qua TTL 5 phút cộng 5 giây rồi in `CAPABILITY_DRAIN_OK realtimeTokensExpired=true`; tiếp theo controller quét residue lần cuối, ghi lifecycle `complete`, và cuối cùng in `FINAL_RESIDUE_OK users=0 profiles=0 restaurants=0 relations=0`. Sau đó inventory độc lập xác nhận `0` user/profile/restaurant/order/GPS.

Nếu owner process bị kill trước hoặc trong capability drain, không provision lại cùng run ID. Chạy lại bằng confirmation exact, run ID cũ và `FOODFLOW_PRODUCTION_SMOKE_MODE=cleanup`; mode này không cần mật khẩu hay signal path cũ. Phải thấy `RECOVERY_CLEANUP`, `CLEANUP_OK`, `CAPABILITY_DRAIN_OK` rồi `FINAL_RESIDUE_OK`, sau đó kiểm tra inventory zero độc lập. Recovery in `outcome=deleted` khi row còn tồn tại; chỉ được in `outcome=already-deleted` khi durable lifecycle tombstone khớp đúng các UUID cũ. Run ID không có record bền vững luôn bị từ chối thay vì báo thành công giả; advisory lease từ chối cleanup nếu owner còn sống. Nếu provision lỗi sau khi bắt đầu mutation, controller in `RECOVERY_REQUIRED` và chỉ in `RECOVERY_RECONCILED` khi tự đối soát xong dưới lease hợp lệ; `RECOVERY_FAILED` bắt buộc chạy recovery bằng run ID gốc.

Không mở thêm Prisma client trong lúc fixture đang sống; Supabase session pool có thể trả `EMAXCONNSESSION` khi chạm giới hạn 15 client. Xóa mọi biến `FOODFLOW_PRODUCTION_SMOKE_*` và signal file sau khi chạy. Quy trình này chỉ chứng minh auth/RBAC zero-state, không thay thế full smoke có active order/provider/device.
