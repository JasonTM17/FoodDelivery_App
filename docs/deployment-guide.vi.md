...
Full local gate → GitHub Actions current-head xanh → rotate secret + preflight → Supabase → Railway API/worker → Admin/Restaurant → production smoke → `HEAD:master` → Docker immutable.
...
- `SUPABASE_URL`, server-only `SUPABASE_SECRET_KEY`, `SUPABASE_PUBLISHABLE_KEY` cho Realtime API kèm ES256 service JWT, `SUPABASE_REALTIME_JWT_PRIVATE_KEY`, `SUPABASE_REALTIME_JWT_KEY_ID`, `SUPABASE_STORAGE_BUCKET=foodflow-public`
