...
> **2026-07-15 status:** 現在の Railway revision `7a0150567c3e408b8ba045faaf781044257ba05c` は healthy で、`/api/healthz` と `/api/readyz` は 200、DB/Redis/Supabase Storage は up です。過去 migration 3 件の checksum は immutable artifact/Git evidence と exact な承認済み production/source pair で照合済みです。source branch `ea89102d6059d2d9102b73acae892629018b791f` には fail-closed guard、provenance 復元、5 件の focused test がありますが、稼働中の `7a015056` image は guard より古い版です。新しい immutable image の deploy と preflight 確認までは production protection を主張しません。`f2c02ed` の test/image 数値は historical evidence であり、現在の deployment の証明ではありません。Full production certification には controlled-device FCM、Android/iOS background-location matrix、authenticated role journeys、certification scope の payment/messaging/AI/owned-routing integrations が必要です。
