...
Evidence mobile local mới nhất: 2026-07-05 trên `codex/batch4-integration`, runtime code đã verify gồm `d201ce1`. `flutter pub get --enforce-lockfile` pass, `flutter analyze` không có issue, toàn bộ `flutter test` pass 166 test và `dart analyze mobile/packages/api_client` không có issue. Android flavor debug APK build pass cho `flutter build apk --debug --flavor customer -t lib/main_customer.dart` và `flutter build apk --debug --flavor driver -t lib/main_driver.dart`, với package ID và launcher label riêng cho customer/driver. Patch driver map tại `d201ce1` render trip route replay và demand heatmap bằng Google Maps overlays thật từ route/demand coordinates của backend; targeted `dart analyze lib\driver\widgets\route_replay_map.dart lib\driver\widgets\heatmap_canvas.dart`, full `flutter analyze`, full `flutter test`, và driver debug APK build đều pass sau patch. Native Google Maps key chỉ lấy từ env/local xcconfig, còn Android release signing fail-closed cho tới khi có đủ secret `FOODFLOW_UPLOAD_*`. Backend route integrity gates cũng pass local cho cùng cụm map/tracking: `pnpm typecheck`, `pnpm lint`, `pnpm build`, backend tracking suite (5 suite / 41 test) và focused dispatch/tracking regressions (2 suite / 16 test).
...
- Mobile pass `flutter pub get --enforce-lockfile`, `flutter analyze`, full `flutter test` (166 test), `dart analyze mobile/packages/api_client` và Android debug APK build cho cả customer/driver flavor. Current-head driver map evidence tại `d201ce1` cũng pass targeted `dart analyze` cho Google Maps widgets, full `flutter analyze`, full `flutter test` và `flutter build apk --debug --flavor driver -t lib/main_driver.dart`. APK build chỉ có warning tương thích tương lai từ `share_plus` áp dụng Kotlin Gradle Plugin, không làm fail.
