...
Evidence map/tracking current-head mới nhất: 2026-07-04 tại `3252c6a`, backend `pnpm install --frozen-lockfile`, `pnpm typecheck`, `pnpm lint`, full `pnpm test` (106 suites / 752 test), focused `route-utils.spec.ts` (11 test) và `pnpm build` đều pass. Admin driver map realtime pass `pnpm --filter foodflow-admin test` (34 files / 139 test), `pnpm --filter foodflow-admin typecheck`, `pnpm --filter foodflow-admin lint`, web `pnpm install --frozen-lockfile` và `pnpm --filter foodflow-admin build`. Mobile customer/driver map hiện dùng chung guard tọa độ giao hàng Việt Nam trước khi vẽ marker, telemetry trail, route polyline hoặc mở Google Maps directions; focused Flutter map/route tests pass 14/14, `flutter analyze` không có issue và full `flutter test` pass 143 test.
...
Evidence backend/web gate current-head mới nhất: 2026-07-04 tại `03ffed4`, backend `pnpm audit`, `pnpm install --frozen-lockfile`, `pnpm prisma generate`, `pnpm typecheck`, `pnpm lint`, full `pnpm test` (106 suites / 752 test) và `pnpm build` đều pass sau các patch dependency advisory. Web workspace `pnpm audit --audit-level moderate`, `pnpm install --frozen-lockfile`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (Admin 34 files / 139 test; Restaurant 27 files / 79 test) và `pnpm build` đều pass sau migration bảo mật Next 15 / next-intl 4. Admin build sinh 70 page localized và Restaurant build sinh 55 page localized cho `vi`, `en`, `ja`.
...
Evidence mobile local mới nhất: 2026-07-04 trên `codex/batch4-integration`, `flutter pub get --enforce-lockfile` pass, focused map/route tests pass 14/14 (`directions_uri_test.dart`, `trip_route_provider_test.dart`, `encoded_polyline_test.dart`, `tracking_provider_test.dart`, `lat_lng_validation_test.dart`), `flutter analyze` không có issue và toàn bộ `flutter test` pass 143 test. Android/iOS Flutter platform scaffold hiện đã có; Android debug APK build pass cho `flutter build apk --debug -t lib/main_customer.dart` và `flutter build apk --debug -t lib/main_driver.dart`. Native Google Maps key chỉ lấy từ env/local xcconfig, còn Android release signing fail-closed cho tới khi có đủ secret `FOODFLOW_UPLOAD_*`. Backend route integrity gates cũng pass local cho cùng cụm map/tracking: `pnpm typecheck`, `pnpm lint`, full `pnpm test` (106 suites, 752 test), focused `route-utils.spec.ts` và `pnpm build`.
