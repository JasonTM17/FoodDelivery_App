...
| Browser E2E | Ma trận Docker isolated được cấu hình 68 check cho mỗi Chromium, Firefox và Pixel 5 (204 tổng). Phải chạy lại trên stack current-source mới; evidence Docker cũ chỉ là lịch sử, không phải bằng chứng first-pass hay production. |
| FCM live send | Chưa chạy: cần project credential production và controlled device token. |

### Record Docker E2E và RAG lịch sử — 13/07/2026

Repository track 33 migration. Một Docker test volume lịch sử tái sử dụng còn 34 dòng `_prisma_migrations` đã apply và một dòng rolled back vì từng ghi một migration zero-step đã bị loại khỏi source; volume này không phải evidence current-source hay migration fresh. Supabase production lần cuối khớp checksum tại 33 migration được track. DB dùng một lần mới và Railway migrator rollout được ủy quyền vẫn là điều kiện cần cho release hiện tại. Thiết kế overlay chạy worker riêng từ backend image, không dùng API process xử lý nền. Log worker lịch sử ghi `FoodFlow Worker started`, lịch RAG `300000ms`, và một lượt sync `indexed: 402`, `unchanged: 0`, `failed: 0`, `deactivated: 0`; phải chạy lại log gate này sau rebuild mới.

Record lịch sử mô tả 352 menu và 50 restaurant document từ các dòng sống E2E. Cả 402 có content hash nhưng không có embedding do chưa cấu hình DeepSeek key. Volume tái sử dụng còn 44 FAQ và 8 policy row có source ID rỗng từ lần local cũ; source hiện tại không có generator cho chúng nên không tính vào evidence worker fresh. Không dữ liệu nào ở đây là production hoặc phê duyệt embedding/provider production.

Ma trận browser được cấu hình 68 check cho mỗi Chromium, Firefox và Pixel 5 (204 tổng). Phải chạy lại trên stack API/Admin/Restaurant current-source mới. Evidence Docker browser trước đây, gồm retry sau `ERR_EMPTY_RESPONSE` ở Admin `/export-jobs`, chỉ là lịch sử: không phải kết quả 204 check first-pass, kết luận UI pass/fail hay production evidence. Các lần kiểm tra trực tiếp Admin/Restaurant desktop/mobile và hai sửa accessibility trước đây cũng chỉ là evidence lịch sử tới khi rebuild được chạy lại. Image local mang `revision=local`, nên một lượt chạy mới chỉ chứng minh checkout hiện tại chứ không chứng minh artifact immutable. CI remote được cấu hình dùng cùng Compose overlay và ma trận ba project, nhưng vẫn cần một lượt remote được ủy quyền chạy mới.
