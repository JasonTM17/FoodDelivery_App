...
`c29c069ea180ed6c3107411759b8ceb2150dc8e7`; the restored source includes the
required final newline. The read-only audit passes all 42 active records, with a
Supabase backup retained outside the repository before the no-op rollout.
`prisma migrate resolve` remains prohibited for future checksum drift.
