...
The Prisma schema declares 60 models across 42 ordered migrations. PostGIS geometry is used for addresses, restaurants, delivery tasks, and location history; the tracked RAG migrations add pgvector-backed storage, a cosine HNSW index, content hashes, and source lookup indexes. Migrations 34–38 cover FCM revocation, public Storage-listing removal, scoped registration revocation, the single-default-address invariant, and the database UUID default. Migrations 39–41 remove empty legacy buckets, add spatial lookup indexes, and verify bucket cleanup. Migration 42 adds a non-secret `production_role_smoke_runs` lifecycle/tombstone table plus restrictive foreign keys and indexes for semantic restaurant approver, cart restaurant, promotion creator, chat sender, support macro creator, and CSAT user references. It preflights all six references and wraps the complete DDL in one explicit transaction. Production SHA `977d55f19ddc4fecafb8a758d2df034f4b6ff21d` has all 42 source migrations active; the 46-row history retains four rolled-back audit rows. Exact Realtime and Job provenance passes the checksum guard, while unrecovered Storage migration bytes intentionally keep the next migrator rollout fail-closed. Current Railway API/readiness and both public Admin/Restaurant web health routes return exact SHA `977d55f`; Database, Redis, and Supabase Storage are ready. A synthetic current-revision GPS/private Broadcast/PostGIS smoke passed in 1,271 ms with exact cleanup, while physical-device and full four-role UI certification remain open. Release SHA `977d55f` is tagged `v0.1.3` on Docker Hub/public GHCR. The remote audit found row-level security on all 60 public application tables, scoped private-Broadcast policies, two expected Storage buckets, and zero production business/GPS fixture data. Supabase flags PostGIS and pgvector in `public`; do not relocate either extension without a separately approved geometry/search-path migration. `realtime_outbox`, `job_outbox`, durable payment webhook/refund records, `dispatch_offers`, private driver KYC submissions, and `ai_usage_events` support the managed-production topology.
...
Schema end-state cannot substitute for source provenance. All 42 source migrations
are active, and `prisma migrate resolve` must not conceal the blocker before the
next migrator rollout.
