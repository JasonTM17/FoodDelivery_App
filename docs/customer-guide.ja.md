...
Current captures は Android API 35 x86_64 AVD の Flutter debug APK で作成しました。Public-auth GIF には credential を入力していません。Authenticated local fixture capture は fixed simulated GPS を使いますが、UI には一般的な current-location label だけを表示し、Home data、backend status `restaurant_pending` の Vietnamese 表示 `Chờ nhà hàng`、Profile rendering を確認します。Mapper と focused tests は backend の 15 個の `OrderStatus` を English、Vietnamese、Japanese でカバーし、未知の future status には localized fallback を使います。Retained dirty-working-tree media は privacy-reviewed regression/product evidence であり、mobile release、payment、map/routing、Firebase、Supabase/Railway、production の証拠ではありません。
