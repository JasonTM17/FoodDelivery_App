...
3. サインイン成功後、welcome、位置情報、通知の onboarding を進めます。permission を拒否しても、位置情報や push 成功を偽装してはいけません。
...
[product gallery](./product-gallery.ja.md#customer) には simulated GPS を使う Android API 35 emulator から privacy review 済みの Customer discovery test-only still があります。これは dirty workspace の regression evidence であり、release/production の証拠ではありません。Admin/Restaurant GIF はそれぞれの web flow 用で、Customer UI として再表示していません。Customer GIF はまだありません。
