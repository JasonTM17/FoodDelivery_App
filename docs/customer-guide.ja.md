...
Current capture は isolated local E2E stack 接続の Android API 35 x86_64 AVD で Flutter debug APK を使いました。Exact simulated coordinates を表示した authenticated Customer stills は docs から除外しました。Retained dirty-working-tree media は privacy-reviewed regression/product evidence であり、mobile release、payment、map/routing、Firebase、Supabase/Railway、production の証拠ではありません。
