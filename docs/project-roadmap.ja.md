...
- Vercel Admin と Restaurant の canonical production aliases は Ready で、SHA `a703ece61e66dcfe7f308cbf46a98098983233e7` を返します。Public `vi/en/ja` login smoke と authenticated GPS/Supabase smoke は pass。より広い authenticated role journeys は pending です。
- Docker Hub と public GHCR の SHA、`v0.1.2`、`latest` aliases は 4 runtime images すべてで digest が一致します。Docker Publish run `29474270122` と Release run `29478484699` が promoted manifests を検証しました。
