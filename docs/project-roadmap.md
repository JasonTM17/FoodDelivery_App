...
Status on 2026-07-16: **runtime SHA `a703ece61e66dcfe7f308cbf46a98098983233e7` is deployed across Railway API/worker/migrator and both Vercel apps; exact-revision health and the 41-migration Supabase state are green, while the Admin/Restaurant Chrome and Customer/Driver API role smoke belongs to historical SHA `17584153` and full production certification remains no-go**.
...
- Admin and Restaurant canonical health return exact SHA `a703ece61e66dcfe7f308cbf46a98098983233e7`. The controlled Chrome zero-state journeys and Customer/Driver read-only API auth were captured at historical SHA `17584153`; current-revision authenticated rerun, native mobile UI, and active-order journeys remain pending.
