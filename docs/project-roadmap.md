...
- Deployed SHA `977d55f` has 41 applied migrations; Database, Redis, and Supabase Storage are ready. Exact Realtime and Job production checksums were recovered from immutable migrator image revision `1f761a65`; their reviewed differences are line endings and one non-executable Job host comment. Storage checksum `4664ac4299eea854a16316be6a9ed689a3320c1fca2557a4fd00f011368fd8e6` was not found in Git objects or inspected images, so `20260712143000_add_production_storage_bucket` is the sole fail-closed provenance blocker. Schema end-state is insufficient and `prisma migrate resolve` must not conceal it. Candidate migration 42 remains undeployed pending provenance recovery, PR review, backup, and synchronized rollout.
