...
- Vercel Admin `dpl_7CFZKPxtNsYeF1Y6BZmnoJEoXyiF` và Restaurant `dpl_6jqguNYtbVCMVaQ6GvikiceYVsGN` là hai exact deployment của SHA `a703ece61e66dcfe7f308cbf46a98098983233e7`. Health canonical trả revision này và public login smoke `vi/en/ja` pass; smoke GPS/Supabase có xác thực đã pass, nhưng lần recheck trực tiếp cho thấy public Restaurant bị Vercel SSO chuyển hướng. Journey bốn role Chrome/API vẫn là evidence lịch sử ở SHA `17584153`; lần chạy lại trên revision hiện tại còn chờ.
- Các alias SHA, `v0.1.2` và `latest` trên Docker Hub lẫn GHCR public khớp digest cho cả bốn image runtime; Docker Publish run `29474270122` và Release run `29477368072` đã xác minh manifest được promote. GHCR semver/`latest` không được tuyên bố vì thao tác ghi manifest trả `401 Unauthorized`.
