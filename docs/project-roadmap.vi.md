...
- SHA `977d55f` đang deploy có 41 migration đã apply; Database, Redis và Supabase Storage đều ready. Checksum production Realtime và Job chính xác đã được khôi phục từ image migrator immutable revision `1f761a65`; khác biệt đã review chỉ là xuống dòng và một comment host Job không thực thi. Checksum Storage `4664ac4299eea854a16316be6a9ed689a3320c1fca2557a4fd00f011368fd8e6` không có trong Git object hay image đã kiểm tra, nên `20260712143000_add_production_storage_bucket` là blocker provenance fail-closed duy nhất. Schema end-state không đủ và không được dùng `prisma migrate resolve` để che. Migration ứng viên 42 vẫn chưa deploy trong khi chờ khôi phục provenance, review PR, backup và rollout đồng bộ.
