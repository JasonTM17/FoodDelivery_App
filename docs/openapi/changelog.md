...
- 2026-07-12: Moved the managed API deployment target from Vercel to Railway. `GET|POST /jobs/drain` remain authenticated recovery/one-off contracts; the Railway worker owns normal Postgres outbox draining. Realtime clients use private Broadcast channels, not `postgres_changes` rows.
