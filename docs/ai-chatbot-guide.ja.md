...
FoodFlow は Batch 4 で LLM-first の chatbot path を使います。Runtime は backend AI module が持ち、外部 automation runner と public AI-tool route は使用しません。
...
- Customer mobile chat は `POST /ai/chat` を直接呼びます。Support-only chat は `orderId` を送らず、order chat は backend grounding 用に有効な order code または UUID だけを送ります。
...
- Chat turns は Admin monitor 用に `ChatSession`/`ChatMessage` へ best-effort で永続化します。Order link は customer-scoped ownership check 後にだけ保存します。
- AI-created support tickets は `channel: ai_chat` と `ai_session:<uuid>` tag を使い、escalation attribution を推測に頼りません。
- High-severity AI escalation は active admin users に persisted notification を作成します。Active admin がいない場合、fake success ではなく `notified: false` と `notifiedAdminCount: 0` を返します。
...
- Food recommendation は authenticated user の order history と、現在 available/open/approved の menu item を組み合わせます。Empty results の場合は捏造せず、verified recommendation がまだ無いと伝えます。
...
- Admin AI monitor は online、not-configured、degraded states を確認します。Conversation counts、escalations、resolution rate は DB-backed で、cost/token latency fields は real usage telemetry source 接続まで null のままです。
