...
> **Release status — 2026-07-15:** current Railway revision `7a0150567c3e408b8ba045faaf781044257ba05c` is healthy: `/api/healthz` and `/api/readyz` return 200 with database, Redis, and Supabase Storage up. The three historical checksum records are reconciled to immutable artifact/Git evidence through exact approved production/source pairs. Source branch `ea89102d6059d2d9102b73acae892629018b791f` adds the fail-closed guard, restored provenance, and focused 5-test coverage; the running `7a015056` image predates that guard, so production protection is not claimed until a new immutable image is deployed and preflighted. Historical `f2c02ed` test/image counts remain dated evidence, not proof of the current deployment. This is **not** full production certification: controlled-device FCM, the Android/iOS background-location matrix, authenticated role journeys, and configured payment/messaging/AI/owned-routing integrations still need evidence.
