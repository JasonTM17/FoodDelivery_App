...
Latest local Batch 4 evidence is recorded in [docs/batch4-release-report.md](docs/batch4-release-report.md), with remote branch cleanup rechecked at `118459e539eecb2dbd61e033431b7f4b5104f0e0`. Latest local code head before the current docs refresh is `188a256`; the clean integration branch remains a fast-forward candidate for `master`. The latest current-source Docker refresh on 2026-07-08 rebuilt Backend/Admin/Restaurant, passed all three health checks, and passed Playwright Chromium/Firefox 70/70 with axe serious/critical smoke, visual contract, realtime, and tenant isolation coverage. Production deploy is still blocked by missing current-head remote CI approval, missing Supabase CLI release env (`SUPABASE_ACCESS_TOKEN`, project ref, database URLs), missing Vercel Admin public env (`NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_WS_URL`, `NEXT_PUBLIC_ADMIN_URL`, `NEXT_PUBLIC_GOOGLE_MAPS_KEY`), and unverified/rotated production secrets.
...
- `master` is the only live remote branch; latest audit on 2026-07-07 showed `origin/master` at `118459e539eecb2dbd61e033431b7f4b5104f0e0`.
